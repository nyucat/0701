...
namespace player_control
{
...

    {
        ...
        public void move()
        {
            ...
            // 检查是否在移动范围内，允许一定的缓冲距离
            float buffer = 0.1f; // 设置一个缓冲距离
            bool withinBounds = (transform.position.x > -37 - buffer &&
                                 transform.position.x < 37 + buffer &&
                                 transform.position.y < 15 + buffer &&
                                 transform.position.y > -15 - buffer);
            ...
            {
                // 计算在边界外的位置，避免速度为0的卡顿
                float clampedX = Mathf.Clamp(transform.position.x, -37, 37);
                float clampedY = Mathf.Clamp(transform.position.y, -15, 15);
            ...
            player_animator.SetFloat("Speed", Direction.magnitude);
        }
        #endregion
